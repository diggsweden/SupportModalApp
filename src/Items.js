/* eslint-disable no-undef */
import chatIcon from '@/assets/chat-smile-2-line.svg';
import supportIcon from '@/assets/customer-service-2-fill.svg';
import faqIcon from '@/assets/questionnaire-line.svg';
import contactIcon from '@/assets/contacts-line.svg';

  /* POC 1 */

  export default [
    { name: 'Öppna Chatten', svgIcon: chatIcon, show: true },
    { name: 'Till Support Sidan', svgIcon: supportIcon, show: true },
    { name: 'Till Frågor och Svar', svgIcon: faqIcon, show: true },
    { name: 'Kontakta Oss', svgIcon: contactIcon, show: true }
  ]
 
  /* POC 2 */

/*
export default [
    {
        title: 'Support',
        content: [
            { name: 'Öppna Chatten', svgIcon: chatIcon, show: true },
    { name: 'Till Support Sidan', svgIcon: supportIcon, show: true },
    { name: 'Till Frågor och Svar', svgIcon: faqIcon, show: true },
    { name: 'Kontakta Oss', svgIcon: contactIcon, show: true }
        ]
    },
    {
        title: 'Tillgänglighet',
        content: [
            { name: 'Ljud', icon: 'sound icon', show: true },
            { name: 'Monochrom färg', icon: 'color icon', show: true },
            { name: 'Stor text', icon: 'text icon', show: true },
            { name: 'High Contrast', icon: 'contrast icon', show: true },
            { name: 'Läs upp text', icon: 'speech icon', show: true },
            { name: 'Text till tal', icon: 'speech icon', show: true }
        ]
    },
    {
        title: 'Change Language',
        content: [
            { name: 'English', icon: 'english icon' , show: true},
            { name: 'Svenska', icon: 'swedish icon', show: true },
        ]
    }
];

*/